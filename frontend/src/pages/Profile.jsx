...
function Profile() {
  ...
  const countryCode = getCode(userInfo.country);
  ...
  const handleUserInfoChange = (event) => {
    ...
    if (name === "languagesSpoken" || name === "interests") {
      const valuesArray = value.split(",").map((item) => item); // Temporarily keep spaces
      newValues[name] = valuesArray;
    } else {
  ...
  const prepareDataForDatabase = (userInfo) => {
